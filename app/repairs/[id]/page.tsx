...
export default function RepairDetailPage() {
  ...
  const printRepairTicket = async () => {
    ...
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Courier New',monospace;background:#e5e7eb;display:flex;justify-content:center;padding:16px}.wrapper{display:flex;flex-direction:column;align-items:center;width:120mm}.tech-card{width:120mm;background:#fff;border-radius:4mm 4mm 0 0;padding:5mm;border:1px solid #c7d2fe;border-bottom:none}.tech-header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:1px solid #e0e7ff;padding-bottom:2.5mm;margin-bottom:2.5mm}.tech-header-left h2{font-size:16px;font-weight:900;color:#1e3a8a}.tech-header-left p{font-size:11px;color:#64748b}.badge-tech{background:#1e3a8a;color:#fff;font-size:11px;font-weight:700;padding:1.5mm 3mm;border-radius:2mm}.ticket-id{text-align:center;font-size:28px;font-weight:900;color:#1e3a8a;padding:2.5mm 0;border-bottom:1px dashed #c7d2fe;margin-bottom:2.5mm}.row{display:flex;justify-content:space-between;gap:3mm}.info-block{flex:1;font-size:12px;line-height:1.55}.lbl{font-weight:700;color:#334155;font-size:10.5px;text-transform:uppercase}.code-box{font-size:11px;padding:2mm 2.5mm;border-radius:2mm;margin-top:2.5mm;border-left:3px solid #22c55e;background:#f0fdf4;color:#166534}.qr-tech-area{text-align:center;min-width:34mm}.qr-tech-area img{width:34mm;height:34mm;display:block}.qr-tech-label{font-size:9px;color:#1e3a8a;font-weight:700;text-align:center;margin-top:1.5mm}.tech-footer{display:flex;justify-content:space-between;font-size:10.5px;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:2mm;margin-top:2.5mm}.cut{width:120mm;text-align:center;font-size:11px;color:#9ca3af;letter-spacing:3px;padding:2.5mm 0;border-top:1px dashed #9ca3af;border-bottom:1px dashed #9ca3af;background:#f9fafb}.client-card{width:120mm;background:#fff;border-radius:0 0 4mm 4mm;padding:4mm 5mm;border:1px solid #bbf7d0;border-top:none}.client-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:2.5mm}.client-header-title{font-size:14px;font-weight:900;color:#166534}.badge-client{background:#166534;color:#fff;font-size:11px;font-weight:700;padding:1.5mm 3mm;border-radius:2mm}.client-info-row{display:flex;justify-content:space-between;gap:3mm}.client-info-block{flex:1;font-size:12px;line-height:1.6}.client-code-big{font-size:21px;font-weight:900;color:#166534;letter-spacing:1px;margin:1.5mm 0}.qr-client-area{text-align:center;min-width:34mm}.qr-client-area img{width:34mm;height:34mm;display:block}.qr-client-label{font-size:9px;color:#166534;font-weight:700;text-align:center;margin-top:1.5mm}@media print{body{background:#fff;padding:0}.no-print{display:none!important}.cut{page-break-after:always;break-after:page}}.no-print{text-align:center;margin-top:6mm;display:flex;gap:3mm;justify-content:center}button{padding:3mm 6mm;background:#1e3a8a;color:#fff;border:none;border-radius:3mm;font-size:14px;cursor:pointer}button.close{background:#64748b}</style></head><body><div class="wrapper">
    ...
    <div class="row"><div class="info-block"><div style="font-size:15px;font-weight:900;color:#1e293b">${escapeHtml(client.name)}</div><div style="font-size:14px;font-weight:800;color:#1e3a8a;margin-top:2mm">${escapeHtml(repair.device)}</div><div style="font-size:13px;font-weight:700;color:#374151;margin-top:1.5mm;margin-bottom:2.5mm">${escapeHtml(repair.issue)}</div><div style="font-size:11px;color:#64748b">${escapeHtml(client.phone || "")}</div>${codeValue ? `<div class="code-box">🔑 Code : ${escapeHtml(codeValue)}</div>` : ""}</div>
    ...
    <div class="client-card"><div class="client-header"><div class="client-header-title">🎫 MBX-${repair.id}<span style="font-size:11px;font-weight:normal;color:#64748b;display:block">${escapeHtml(repair.device)} · ${dateStr}</span></div><span class="badge-client">CLIENT</span></div>
    <div class="client-info-row"><div class="client-info-block"><div><span class="lbl">Client</span><br>${escapeHtml(client.name)}</div><div style="margin-top:2.5mm"><span class="lbl">Panne déclarée</span><br>${escapeHtml(repair.issue)}</div>${repair.estimated_price ? `<div style="margin-top:2.5mm"><span class="lbl">Prix estimé</span><br><span style="font-size:16px;font-weight:900;color:#166534">${Number(repair.estimated_price).toFixed(2)} €</span></div>` : ""}${!codeValue ? `<div style="font-size:10.5px;background:#fef2f2;border-left:3px solid #ef4444;color:#991b1b;font-weight:700;padding:2mm 2.5mm;border-radius:2mm;margin-top:2.5mm">⚠️ Appareil non testé — pas pris en garantie</div>` : ""}<div style="margin-top:2.5mm"><span class="lbl">Code de suivi</span><div class="client-code-big">${client.client_code || "—"}</div><span style="font-size:10px;color:#64748b">→ technophone.vercel.app/suivi-client</span></div></div>
