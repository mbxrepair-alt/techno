...
export default function RepairDetailPage() {
  ...
  const quickActions = [
    { status: "📤 Envoyé à l'atelier", label: "Envoyé", color: "bg-blue-500", textColor: "text-white" },
    { label: "⏳ Attente validation client", status: "⏳ Attente validation client", color: "bg-amber-500", textColor: "text-white" },
    { label: "🔐 Mot de passe incorrect", status: "🔐 Mot de passe incorrect", color: "bg-red-500", textColor: "text-white" },
    { label: "📦 Attente pièce", status: "📦 Attente pièce", color: "bg-orange-500", textColor: "text-white" },
    { label: "🔬 Diagnostic", status: "🔬 Diagnostic", color: "bg-blue-500", textColor: "text-white" },
    { label: "🔧 En réparation", status: "🔧 En réparation", color: "bg-orange-600", textColor: "text-white" },
    ...
    { label: "🚫 Refus client", status: "🚫 Refus client", color: "bg-pink-600", textColor: "text-white" },
  ];
  ...
  const loadAllDataForSearch = async () => {
    try {
      const user = await getCurrentUser();
      if (!user) return;
      const { data: repairsData } = await supabase.from("repairs").select("*").eq("user_id", user.id).order("id", { ascending: false });
  ...
  const loadRepair = async () => {
    ...
    try {
      const user = await getCurrentUser();
      if (!user) return;
      const { data, error } = await supabase.from("repairs").select("*").eq("id", id).eq("user_id", user.id).single();
      ...
      if (data.parts_used) { try { setParts(JSON.parse(data.parts_used)); } catch { setParts([]); } }
  ...
  const autoSave = async () => {
    ...
    if (diagnosticTechnicien !== lastSavedRef.current.diagnostic && diagnosticTechnicien !== "") {
      await ajouterHistorique('modification', `📝 Diagnostic technicien modifié`, lastSavedRef.current.diagnostic || "(vide)", diagnosticTechnicien);
    ...
    if (repairDescription !== lastSavedRef.current.repairDescription && repairDescription !== "") {
      await ajouterHistorique('modification', `🔧 Travaux effectués modifiés`, lastSavedRef.current.repairDescription || "(vide)", repairDescription);
      lastSavedRef.current.repairDescription = repairDescription;
    }
    
    if (risks !== lastSavedRef.current.risks && risks !== "") {
      await ajouterHistorique('modification', `⚠️ Risques modifiés`, lastSavedRef.current.risks || "(vide)", risks);
      lastSavedRef.current.risks = risks;
    }
    
    if (testsPassed !== lastSavedRef.current.testsPassed && testsPassed !== "") {
      await ajouterHistorique('modification', `✅ Tests modifiés`, lastSavedRef.current.testsPassed || "(vide)", testsPassed);
  ...
  const handleSearch = (value) => {
    ...
    const results = filtered.map(repair => ({ ...repair, clientName: clientMap.get(repair.client_id)?.name || "Client inconnu" }));
  ...
  const uploadPhoto = async (e) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { showMessage("Veuillez sélectionner une image", "error"); return; }
    if (file.size > 5 * 1024 * 1024) { showMessage("L'image ne doit pas dépasser 5Mo", "error"); return; }
    setUploading(true);
    try {
      const user = await getCurrentUser();
      if (!user) return;
  ...
  const updateStatus = async (newStatus) => {
    ...
    if (error) {
      ...
      await ajouterHistorique('changement_statut', `🔄 Statut : "${oldStatus}" → "${newStatus}"`, oldStatus, newStatus);
  ...
  const addPart = async () => {
    ...
    await ajouterHistorique('modification', `🔩 Pièce ajoutée : ${currentPart.name} x${currentPart.quantity} (${currentPart.price * currentPart.quantity}€)`);
  ...
  const sendEmailReceipt = async () => {
    if (!emailTo || !emailTo.trim()) { showMessage("Aucun email client", "error"); return; }
    setSendingEmail(true);
    try {
      const repairsHtml = `<div>🔧 Ticket #${repair.id}<br/>📱 ${repair.device}<br/>🔧 ${repair.issue}<br/>💰 ${finalPrice || 0}€</div>`;
      const templateParams = {
        company_name: "MBX Réparations", company_address: "", company_phone: "", company_email: "",
        client_name: client?.name || "Client", client_phone: client?.phone || "Non renseigné",
        client_email: emailTo, date: new Date().toLocaleString('fr-FR'),
        repairs_html: repairsHtml, to_email: emailTo
      };
        ...
        <div className="w-full h-full relative bg-gray-800 rounded-lg border border-gray-700 shadow-sm">
          <svg className="absolute inset-0 w-full h-full">
            {getLines().map((line, idx) => (
              <line key={idx} x1={`${line.from.x}%`} y1={`${line.from.y}%`} x2={`${line.to.x}%`} y2={`${line.to.y}%`} stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" />
            ))}
          </svg>
          {points.map(point => {
            ...
            return (
              <div 
                ...
                className={`absolute w-3.5 h-3.5 -ml-1.5 -mt-1.5 rounded-full transition-all duration-200 ${
                  isSelected 
                    ? (isStart ? 'bg-green-500 ring-2 ring-green-300 shadow-[0_0_10px_rgba(34,197,94,0.6)]' : 
                       isEnd ? 'bg-red-500 ring-2 ring-red-300 shadow-[0_0_10px_rgba(239,68,68,0.6)]' : 
  ...
  return (
    <Layout>
      {message.text && (
        <div className={`fixed bottom-6 right-6 px-4 py-2 rounded-lg text-white shadow-lg z-50 text-sm ${message.type === "error" ? "bg-red-500" : "bg-green-500"}`}>
          {message.text}
        </div>
      )}
      ...
      {showPhotoModal && selectedPhoto && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setShowPhotoModal(false)}>
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setShowPhotoModal(false)} className="absolute top-2 right-2 bg-black/50 text-white rounded-full w-8 h-8 flex items-center justify-center">✕</button>
          </div>
        </div>
      )}
      ...
      {showEmailModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-5">
            ...
            <input type="email" className="w-full border rounded-lg p-2 mb-3" value={emailTo} onChange={(e) => setEmailTo(e.target.value)} placeholder="Email du client" />
            <div className="flex gap-2">
              <button onClick={sendEmailReceipt} disabled={sendingEmail} className="flex-1 bg-green-600 text-white py-2 rounded-lg">{sendingEmail ? "Envoi..." : "Envoyer"}</button>
              <button onClick={() => setShowEmailModal(false)} className="flex-1 bg-gray-200 py-2 rounded-lg">Annuler</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-5">
            ...
            <div className="grid grid-cols-1 gap-2">
              {quickActions.map((action, idx) => (
                <button key={idx} onClick={() => { updateStatus(action.status); setShowQuickActions(false); }} className={`${action.color} ${action.textColor} py-2 rounded-lg text-sm font-semibold`}>
                  {action.label}
                </button>
              ))}
            </div>
      ...
      {showPartModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-5">
            ...
            <input type="text" placeholder="Nom" className="w-full border rounded-lg p-2 mb-2" value={currentPart.name} onChange={(e) => setCurrentPart({ ...currentPart, name: e.target.value })} />
            <div className="flex gap-2 mb-3">
              <input type="number" placeholder="Qté" className="flex-1 border rounded-lg p-2" value={currentPart.quantity} onChange={(e) => setCurrentPart({ ...currentPart, quantity: parseInt(e.target.value) || 1 })} />
              <input type="number" placeholder="Prix" className="flex-1 border rounded-lg p-2" value={currentPart.price} onChange={(e) => setCurrentPart({ ...currentPart, price: parseFloat(e.target.value) || 0 })} />
            </div>
            <div className="flex gap-2">
              <button onClick={addPart} className="flex-1 bg-blue-600 text-white py-2 rounded-lg">Ajouter</button>
              <button onClick={() => setShowPartModal(false)} className="flex-1 bg-gray-200 py-2 rounded-lg">Annuler</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="w-full mx-auto px-0 py-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border p-3">
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <input type="text" value={searchTerm} onChange={(e) => handleSearch(e.target.value)} placeholder="🔍 Rechercher par nom client ou numéro de ticket..." className="w-full p-2 pl-8 border rounded-lg text-sm" />
                <div className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400 text-sm">🔍</div>
              </div>
              <button onClick={() => setShowSearchResults(false)} className="px-3 py-2 bg-gray-100 rounded-lg text-sm">✕</button>
            </div>
            {showSearchResults && searchResults.length > 0 && (
              <div className="absolute z-20 left-0 right-0 mt-1 bg-white border rounded-lg shadow-lg max-h-60 overflow-auto">
                {searchResults.map((result) => (
                  <div key={result.id} onClick={() => goToRepair(result.id)} className="p-2 hover:bg-gray-50 cursor-pointer border-b flex justify-between items-center text-sm">
                    <div>
                    ...
                    <button className="bg-blue-600 text-white px-3 py-1 rounded text-xs">Voir</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="p-6">
            ...
            <div className="flex flex-wrap justify-between items-start gap-3 mb-4">
              <div className="flex items-center gap-3 flex-wrap">
                <span className="bg-white text-blue-700 font-bold text-2xl px-4 py-2 rounded-xl shadow-lg">🎫 MBX-{id}</span>
                <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                  repair?.status === "✅ Terminé" ? "bg-green-500 text-white" :
                  repair?.status === "🔧 En réparation" ? "bg-orange-500 text-white" :
                  repair?.status === "🔬 Diagnostic" ? "bg-blue-500 text-white" :
                  repair?.status === "📥 Réceptionné" ? "bg-amber-500 text-white" :
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowEmailModal(true)} className="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-lg text-sm font-medium transition">✉️ Email</button>
                <button onClick={() => setShowQuickActions(true)} className="bg-white/20 hover:bg-white/30 text-white w-10 h-10 rounded-lg text-xl font-bold transition">+</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              ...
              <div className="bg-white/10 rounded-lg p-3">
                ...
                <p className="text-white font-bold text-xl">{client?.phone !== "NC" ? client?.phone : "Non renseigné"}</p>
              </div>
              <div className="bg-white/10 rounded-lg p-3">
                <p className="text-blue-200 text-xs mb-1">🔑 CODE DÉVERROUILLAGE</p>
                <div className="flex items-center gap-2">
                  <p className="text-white font-bold text-xl">{repair?.unlock_code !== "NC" ? repair?.unlock_code : "Non fourni"}</p>
              ...
              <div className="bg-white/10 rounded-lg p-3">
                ...
                <p className="text-white font-mono text-lg">{repair?.imei !== "NC" ? repair?.imei : "Non renseigné"}</p>
              </div>
            </div>
                      ...
                      <div 
                        className={`w-10 h-10 rounded-full flex items-center justify-center text-base transition-all duration-300 ${
                          currentStep >= step.step 
                            ? `${step.status === "✅ Terminé" ? "bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.6)]" : 
                               step.status === "🔧 En réparation" ? "bg-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.6)]" : 
                               "bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.6)]"} text-white` 
                            : 'bg-white/20 text-white/50'
                        }`}
                      >
                        {step.icon}
                      </div>
                      <span className={`text-xs mt-2 font-medium ${currentStep >= step.step ? 'text-white' : 'text-white/50'}`}>
                        {step.label}
                      </span>
                    </button>
                  ))}
        ...
        {repair?.client_response && (
          <div className="mb-6 bg-amber-50 border-l-4 border-amber-500 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              {repair.client_response_type === "accepte" && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">Accepté</span>}
              {repair.client_response_type === "refuse" && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded">Refusé</span>}
            </div>
            <p className="text-gray-700 text-sm italic">"{repair.client_response}"</p>
            <p className="text-xs text-gray-400 mt-1">Le {new Date(repair.updated_at).toLocaleString("fr-FR")}</p>
          </div>
        )}
        ...
        <div className="mb-5 flex justify-end">
          ...
          >
            ...
            <span className="text-xs bg-gray-700 px-2 py-0.5 rounded-full">{historique.length}</span>
          </button>
        </div>
                ...
                <div className="space-y-3">
                  {historique.map((entry) => (
                    <div key={entry.id} className={`rounded-lg p-3 border-l-4 ${
                      entry.action === 'changement_statut' ? 'border-purple-500 bg-purple-50' :
                      entry.action === 'changement_technicien' ? 'border-orange-500 bg-orange-50' :
                      entry.action === 'modification' ? 'border-blue-500 bg-blue-50' :
...
💰 Devis estimé:"
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
              <div className="bg-gray-50 px-4 py-2 border-b flex justify-between items-center">
                ...
                <button onClick={() => setShowPartModal(true)} className="text-blue-600 text-xs hover:text-blue-700 flex items-center gap-1">
              ...
              <div className="p-3">
                ...
                {parts.length > 0 && (
                  <div className="mt-3 bg-gray-50 rounded p-2">
                    <h3 className="text-xs font-semibold text-gray-600 mb-1">🔩 Pièces utilisées</h3>
                    {parts.map((part) => (
                      <div key={part.id} className="flex justify-between text-xs py-1">
                        ...
                        <button onClick={() => removePart(part.id)} className="text-red-500 hover:text-red-700">✕</button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
              <div className="bg-gray-50 px-4 py-2 border-b">
                <h2 className="font-semibold text-gray-700 text-sm">✅ Tests & contrôles qualité</h2>
              </div>
              <div className="p-3">
                <textarea 
                  rows={3} 
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
              ...
              <div className="p-3">
                ...
                {repair?.estimated_price && repair?.estimated_price > 0 && !repair?.final_price && (
                  <p className="text-xs text-gray-400 mt-2 text-center">Prix estimé: {repair.estimated_price}€</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
              ...
              <div className="p-3">
                <div className={`px-4 py-3 rounded-lg text-white font-semibold text-center shadow-md ${
                  repair?.status === "✅ Terminé" ? "bg-green-500" : 
                  repair?.status === "🔧 En réparation" ? "bg-orange-500" : 
                  repair?.status === "🔬 Diagnostic" ? "bg-blue-500" : 
                  repair?.status === "📥 Réceptionné" ? "bg-amber-500" : 
                  repair?.status === "🚫 Refus client" ? "bg-pink-500" : 
                  "bg-gray-500"
                }`}>
                  {repair?.status || "📥 Réceptionné"}
                </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
              ...
              <div className="p-3">
                {photos.length === 0 ? (
                  <div className="text-center py-6 text-gray-400 border-2 border-dashed rounded-lg text-sm">📭 Aucune photo</div>
                ) : (
                  <div className="grid grid-cols-3 gap-2">
                    {photos.map((photo, index) => (
                      <div key={index} className="relative aspect-square bg-gray-100 rounded-lg overflow-hidden group">
                        <img src={photo} alt={`Photo ${index + 1}`} className="w-full h-full object-cover cursor-pointer" onClick={() => { setSelectedPhoto(photo); setShowPhotoModal(true); }} />
                        <button onClick={() => deletePhoto(photo)} className="absolute top-1 right-1 bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition">✕</button>
                      </div>
                    ))}
                  </div>
                )}
