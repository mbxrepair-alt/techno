...
function getClosestPoint(x: number, y: number): Point | null {
  ...
  for (const point of POINTS) {
    ...
    if (d < minDistance) { minDistance = d; closest = point; }
  }
  return closest;
}
...
export default function PatternLock({ onComplete, onClear }: PatternLockProps) {
  ...
  const handlePointerEvent = (e: React.MouseEvent | React.TouchEvent, type: 'start' | 'move'): void => {
    e.preventDefault();
  ...
  const handleEnd = (): void => { setIsDrawing(false); setMousePos({ x: 0, y: 0 }); };

  const handleClear = (): void => { setPattern([]); onClear(); };
    ...
    <div className="w-full max-w-[200px] mx-auto">
      ...
      >
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          ...
          {isDrawing && lastPoint && (
            <line x1={`${lastPoint.x}%`} y1={`${lastPoint.y}%`} x2={`${mousePos.x}%`} y2={`${mousePos.y}%`} stroke="#f97316" strokeWidth="3" strokeDasharray="4" strokeLinecap="round" />
          )}
        </svg>
        {POINTS.map(point => (
          <div
      ...
      <div className="flex justify-between items-center mt-2 px-1">
        <button type="button" onClick={handleClear} className="text-[10px] text-gray-400 hover:text-orange-500 transition">Effacer</button>
        <span className="text-[10px] text-gray-400">{pattern.length === 0 ? "Dessinez" : `${pattern.length} pts`}</span>
      </div>
