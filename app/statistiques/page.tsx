  ...
  const loadStats = async () => {
    ...
    try {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        last6Months.push({ month: monthNames[d.getMonth()], year: d.getFullYear(), count: 0, revenue: 0 });
      }
      ...
      const recentRepairs = repairs?.slice(0, 5).map(r => ({
        id: r.id,
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              {isGerant ? "Vision globale de l'atelier" : `Statistiques de ${currentTechnician?.name}`}
            </p>
          </div>
          <select
            value={period}
          ...
          <div className="bg-white rounded-xl shadow-md p-5 border border-gray-100">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={stats.repairsByMonth}>
                ...
                <YAxis />
                <Tooltip />
                <Area type="monotone" dataKey="réparations" stroke="#f97316" fillOpacity={1} fill="url(#colorRepairs)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-5 border border-gray-100">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                ...
                <span className="font-bold text-gray-800">{stats.pendingRepairs + stats.inProgressRepairs}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">⭐ Taux de complétion</span>
                <span className="font-bold text-green-600">{Math.round((stats.completedRepairs / stats.totalRepairs) * 100) || 0}%</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {stats.recentRepairs.map((repair) => (
                    <tr key={repair.id} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/repairs/${repair.id}`)}>
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          ...
                          repair.status === "🔧 En réparation" ? "bg-orange-100 text-orange-800" :
                          "bg-gray-100 text-gray-800"
                        }`}>
                          {repair.status}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right text-sm font-medium text-gray-800">{repair.price}€</td>
                    </tr>
                  ))}
                </tbody>
              </table>
