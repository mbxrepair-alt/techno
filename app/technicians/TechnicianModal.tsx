...
const DEFAULT_FORM: Required<TechnicianData> = {
  name: "", access_code: "", is_active: true,
  can_access_repairs: false, can_access_clients: false, can_access_factures: false,
  can_access_paiements: false, can_access_statistiques: false, can_access_settings: false,
  is_gerant: false,
};
...
export default function TechnicianModal({ isOpen, onClose, onSave, technician }: TechnicianModalProps): JSX.Element | null {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>): void => {
    ...
    if (!formData.access_code || formData.access_code.length !== 4) { alert("Un code à 4 chiffres est requis"); return; }
    onSave(formData);
  };
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold">{technician ? "✏️ Modifier" : "➕ Ajouter un technicien"}</h2>
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl">×</button>
          </div>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Nom complet *</label>
              <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="Jean Dupont" required />
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Code d&apos;accès (4 chiffres) *</label>
              <div className="flex gap-2">
                <input type="text" value={formData.access_code} onChange={(e) => setFormData({ ...formData, access_code: e.target.value.replace(/[^0-9]/g, "").slice(0, 4) })} className="flex-1 px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-center text-2xl tracking-widest" placeholder="1234" maxLength={4} required />
                <button type="button" onClick={generateRandomCode} className="px-3 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 text-sm" title="Générer un code aléatoire">🎲</button>
              </div>
            </div>
            <div className="mb-4">
            ...
            <div className="mb-4">
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formData.is_gerant} onChange={(e) => setFormData({ ...formData, is_gerant: e.target.checked })} />
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" className="flex-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">{technician ? "Mettre à jour" : "Ajouter"}</button>
            </div>
          </form>
        </div>
      </div>
