...
export default function Dashboard(): JSX.Element {
  ...
  const [repairsList, setRepairsList] = useState([
    { device: "", issue: "", imei: "", code: "", estimatedPrice: "", unlockPattern: "", description: "", id: Date.now() }
  ]);
  ...
  const defaultDevices = [
    ...
    "Huawei P60 Pro", "Huawei P50",
    "Oppo Find X6", "Oppo Reno 10"
  ];
  ...
  const defaultIssues = [
    "Écran cassé / brisé", "Batterie ne charge plus / gonflée", "Microphone HS / pas de son",
    "Appareil photo flou / cassé", "Plus de réseau / pas de SIM", "Bootloop / redémarrage en boucle",
    "Bouton power / volume HS", "Tombé dans l'eau / oxydation", "Haut-parleur grésille / pas de son",
    "GPS ne fonctionne pas", "Code oublié / verrouillage", "WiFi / Bluetooth ne marche pas",
    "Port de charge cassé", "Vitre arrière cassée", "Face ID / Touch ID ne fonctionne pas",
    "Overchauffe / téléphone chauffe", "Vibreur HS", "Proximité / écran qui reste noir",
    "Mise à jour bloquée", "Application qui plante"
  ];
  ...
  const defaultCodesList = ["0000", "1234", "1111", "000000", "123456", "654321", "Code oublié", "Non fourni", "Motorola", "Samsung", "Xiaomi", "iPhone"];
  ...
  const handleClientKeyDown = (e) => {
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      if (showClientSuggestions && clientSuggestions.length > 0) {
        setSelectedClientIndex(prev => (prev - 1 + clientSuggestions.length) % clientSuggestions.length);
      }
    } else if (e.key === "Enter") {
      e.preventDefault();
      if (showClientSuggestions && selectedClientIndex >= 0 && clientSuggestions[selectedClientIndex]) {
  ...
  const handlePhoneKeyDown = (e) => {
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      if (showPhoneSuggestions && phoneSuggestions.length > 0) {
        setSelectedPhoneIndex(prev => (prev - 1 + phoneSuggestions.length) % phoneSuggestions.length);
      }
    } else if (e.key === "Enter") {
      e.preventDefault();
  ...
  const generateRepairSlots = () => {
    ...
    for (let i = 0; i < count; i++) {
      newList.push({ device: "", issue: "", imei: "", code: "", estimatedPrice: "", unlockPattern: "", description: "", id: now + i });
    }
    setRepairsList(newList);
  ...
  const searchCodeSuggestions = (value, repairId) => {
    ...
    const filtered = allCodesList.filter(code => 
  ...
  const generateCreditCardTicket = async (ticket, client, trackingUrl = null) => {
    ...
    if (codeValue === "" || codeValue === "NC" || codeValue === null || codeValue === "Non fourni") {
      codeValue = "⚠️ NON FOURNI - Test impossible, pas pris en garantie";
    }
    
    let notesValue = ticket.description || ticket.notes || "";
    let descriptionFinale = notesValue && notesValue !== "" && notesValue !== "NC" ? `📝 ${notesValue}` : "📝 Aucune note spécifique";
  ...
  const sendEmailReceipt = async (tickets, client, recipientEmail, trackingUrl = null) => {
    ...
    try {
      const repairsHtml = tickets.map(t => `
  ...
  const createIntake = async () => {
    ...
    try {
      ...
      if (!existing) {
        ...
        clientData = existing;
        if ((intakePhone && existing.phone !== intakePhone) || (intakeEmail && existing.email !== intakeEmail)) {
          await supabase
            .from("clients")
            .update({ 
            ...
            .eq("id", existing.id);
          clientData = { ...existing, phone: intakePhone || existing.phone, email: intakeEmail || existing.email };
        }
      }
      ...
      setIntakeEmail("");
      setDesiredRepairCount(1);
      setRepairsList([{ device: "", issue: "", imei: "", code: "", estimatedPrice: "", unlockPattern: "", description: "", id: Date.now() }]);
      setClientSuggestions([]);
      setPhoneSuggestions([]);
      setShowClientSuggestions(false);
      setShowPhoneSuggestions(false);
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full">
            ...
            <div className="bg-gray-50 rounded-xl p-3 mb-5 max-h-40 overflow-auto">
              {recentTickets.map((ticket) => (
                <div key={ticket.id} className="text-sm py-2 border-b last:border-0 flex justify-between">
              ...
              <div className="relative flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full text-white shadow-md ${statusColors[r.status] || "bg-yellow-500"}`}>
                      {r.status || "🟡 Réceptionné"}
                    </span>
                  </div>
                  <div className="text-sm text-gray-600 mt-1">{r.client?.name}</div>
                  <div className="text-xs text-gray-400 mt-0.5">📱 {r.device} • 🔧 {r.issue}</div>
                </div>
                <div className="text-blue-500 group-hover:translate-x-1 transition-transform duration-200">→</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-lg">
        ...
        <div className="p-6">
          <div className="space-y-5">
                  ...
                  {showClientSuggestions && clientSuggestions.length > 0 && (
                    <div className="absolute z-10 bg-white border border-blue-200 rounded-xl shadow-lg max-h-48 overflow-auto w-full mt-1">
                      {clientSuggestions.map((c, idx) => (
                        <div key={c.id} className={`p-2 cursor-pointer hover:bg-blue-50 ${selectedClientIndex === idx ? 'bg-blue-50' : ''}`} onMouseDown={() => selectClient(c)}>
                          <div className="font-medium">{c.name}</div>
                          <div className="text-xs text-gray-500">{c.phone !== "NC" ? c.phone : ""}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {showPhoneSuggestions && phoneSuggestions.length > 0 && (
                    <div className="absolute z-10 bg-white border border-blue-200 rounded-xl shadow-lg max-h-48 overflow-auto w-full mt-1">
                      {phoneSuggestions.map((c, idx) => (
                        <div key={c.id} className={`p-2 cursor-pointer hover:bg-blue-50 ${selectedPhoneIndex === idx ? 'bg-blue-50' : ''}`} onMouseDown={() => selectPhoneSuggestion(c)}>
                ...
                {repairsList.map((repair, idx) => (
                  <div key={repair.id} className="border border-gray-200 rounded-xl p-3 bg-white">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-xs font-semibold text-blue-600">Appareil #{idx+1}</span>
                      {repairsList.length > 1 && <button onClick={() => removeRepair(repair.id)} className="text-red-500 text-xs hover:text-red-700 transition">🗑 Supprimer</button>}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="relative">
                        <input 
                        ...
                        {showDeviceSuggestionsMap[repair.id] && deviceSuggestionsMap[repair.id]?.length > 0 && (
                          <div className="absolute z-10 bg-white border border-blue-200 rounded-lg shadow-lg max-h-36 overflow-auto w-full mt-1">
                            {deviceSuggestionsMap[repair.id].map((d, i) => (
                              <div key={i} className="p-2 cursor-pointer hover:bg-blue-50 text-sm" onMouseDown={() => selectDevice(repair.id, d)}>📱 {d}</div>
                            ))}
                          </div>
                        )}
                      </div>
                        ...
                        {showIssueSuggestionsMap[repair.id] && issueSuggestionsMap[repair.id]?.length > 0 && (
                          <div className="absolute z-10 bg-white border border-blue-200 rounded-lg shadow-lg max-h-36 overflow-auto w-full mt-1">
                            {issueSuggestionsMap[repair.id].map((iss, i) => (
                              <div key={i} className="p-2 cursor-pointer hover:bg-blue-50 text-sm" onMouseDown={() => selectIssue(repair.id, iss)}>🔧 {iss}</div>
                            ))}
                          </div>
                        )}
                      </div>
                          ...
                          {showCodeSuggestionsMap[repair.id] && codeSuggestionsMap[repair.id]?.length > 0 && (
                            <div className="absolute z-10 bg-white border border-blue-200 rounded-lg shadow-lg max-h-36 overflow-auto w-full mt-1">
                              {codeSuggestionsMap[repair.id].map((code, i) => (
                                <div key={i} className="p-2 cursor-pointer hover:bg-blue-50 text-sm" onMouseDown={() => selectCodeForRepair(repair.id, code)}>🔑 {code}</div>
                              ))}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="md:col-span-2">
                        <PatternLock onComplete={(pattern) => updateRepairField(repair.id, "unlockPattern", pattern.join("-"))} onClear={() => updateRepairField(repair.id, "unlockPattern", "")} />
                        <p className="text-[10px] text-gray-400 mt-1">⚠️ NON FOURNI - Test impossible, pas pris en garantie</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <button onClick={createIntake} disabled={loading} className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-3 rounded-xl font-bold hover:from-blue-700 hover:to-cyan-700 transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50">
      ...
      {showAddDevice && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-96 border border-blue-200 shadow-xl">
            ...
            <input type="text" className="border border-gray-200 p-2 rounded w-full mb-4 focus:border-blue-400 focus:outline-none" placeholder="Nom du modèle" value={newDeviceInput} onChange={(e) => setNewDeviceInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCustomDevice()} />
            <div className="flex gap-2"><button onClick={addCustomDevice} className="bg-blue-600 text-white px-4 py-2 rounded flex-1 hover:bg-blue-700">Ajouter</button><button onClick={() => setShowAddDevice(false)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded flex-1 hover:bg-gray-300">Annuler</button></div>
          </div>
        </div>
      )}
      ...
      {showAddIssue && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-96 border border-blue-200 shadow-xl">
            ...
            <input type="text" className="border border-gray-200 p-2 rounded w-full mb-4 focus:border-blue-400 focus:outline-none" placeholder="Nom de la panne" value={newIssueInput} onChange={(e) => setNewIssueInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCustomIssue()} />
            <div className="flex gap-2"><button onClick={addCustomIssue} className="bg-blue-600 text-white px-4 py-2 rounded flex-1 hover:bg-blue-700">Ajouter</button><button onClick={() => setShowAddIssue(false)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded flex-1 hover:bg-gray-300">Annuler</button></div>
          </div>
        </div>
      )}
      ...
      {showAddCode && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-96 border border-blue-200 shadow-xl">
            ...
            <input type="text" className="border border-gray-200 p-2 rounded w-full mb-4 focus:border-blue-400 focus:outline-none" placeholder="Code téléphone" value={newCodeInput} onChange={(e) => setNewCodeInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCustomCode()} />
            <div className="flex gap-2"><button onClick={addCustomCode} className="bg-blue-600 text-white px-4 py-2 rounded flex-1 hover:bg-blue-700">Ajouter</button><button onClick={() => setShowAddCode(false)} className="bg-gray-200 text-gray-700 px-4 py-2 rounded flex-1 hover:bg-gray-300">Annuler</button></div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-auto border border-blue-200 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white p-4 border-b border-blue-200">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold text-blue-600">🔧 Détail MBX-{selectedRepairDetail.id}</h2>
                  <p className="text-sm text-gray-500">{selectedRepairClient.name}</p>
                </div>
                <button onClick={() => setShowDetailModal(false)} className="text-gray-400 hover:text-blue-500 text-2xl transition">✕</button>
              </div>
            </div>
            <div className="p-5 space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="font-semibold text-gray-800">👤 {selectedRepairClient.name}</div>
                {selectedRepairClient.phone !== "NC" && <div className="text-sm text-gray-600">📞 {selectedRepairClient.phone}</div>}
                {selectedRepairClient.email !== "NC" && <div className="text-sm text-gray-600">✉️ {selectedRepairClient.email}</div>}
                {selectedRepairClient.client_code && <div className="text-sm font-mono text-blue-600">🔑 Code: {selectedRepairClient.client_code}</div>}
              </div>
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="grid grid-cols-2 border-b border-gray-200"><div className="p-3 bg-gray-50 font-semibold">📱 Appareil</div><div className="p-3">{selectedRepairDetail.device}</div></div>
                <div className="grid grid-cols-2"><div className="p-3 bg-gray-50 font-semibold">🔧 Panne</div><div className="p-3">{selectedRepairDetail.issue}</div></div>
              </div>
              <div className="flex gap-2">
                ...
                }} className="flex-1 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition">🖨️ Ticket</button>
                <button onClick={() => setShowDetailModal(false)} className="flex-1 bg-gray-200 text-gray-700 py-2 rounded-lg hover:bg-gray-300 transition">Fermer</button>
              </div>
            </div>
          </div>
        </div>
