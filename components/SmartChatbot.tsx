...
export default function SmartChatbot({
  ...
  const analyzeSingleDevice = (msg: string): RepairDetection => {
    ...
    if (msg.includes("samsung") || msg.includes("galaxy")) {
      result.device = msg.includes("s23")
        ? "Samsung Galaxy S23"
        : msg.includes("s22")
          ? "Samsung Galaxy S22"
          : "Samsung Galaxy";
    ...
    if (msg.includes("écran") || msg.includes("cassé") || msg.includes("vitre")) {
      ...
      result.estimatedPrice = result.device.includes("iPhone 14")
        ? 229
        : result.device.includes("iPhone 13")
          ? 199
          : result.device.includes("iPhone 12")
            ? 169
            : 89;
  ...
  const analyzeMessage = (message: string): AnalysisResult => {
    ...
    if (multiMatch && msg.includes("+")) {
      const devices = msg
        .split(/[+,et]/)
        .map((part) => analyzeSingleDevice(part))
        .filter((r) => r.device);
  ...
  const sendMessage = async (): Promise<void> => {
    ...
    try {
      ...
      if (analysis.type === "multi" && analysis.repairs?.length) {
        ...
        const list = analysis.repairs
          .map((r, i) => `\n${i + 1}. 📱 ${r.device} - ${r.issue} (${r.estimatedPrice}€)`)
          .join("");
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content: `🔍 **J'ai détecté ${analysis.repairs!.length} appareils :**${list}\n\n✅ Cliquez sur **"Remplir tous les appareils"** ci-dessous.`,
          },
        ]);
    ...
    <div className="fixed bottom-24 right-6 z-50 w-96 bg-white rounded-2xl shadow-2xl border border-orange-200 overflow-hidden animate-slide-up">
      ...
      <div className="h-80 overflow-y-auto p-4 bg-gray-50">
        {messages.map((msg, idx) => (
          ...
          >
            <div
              className={`max-w-[85%] p-3 rounded-xl text-sm ${msg.role === "user" ? "bg-orange-500 text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none shadow"}`}
            >
      ...
      <div className="px-3 py-2 bg-gray-50 border-t">
        <div className="flex flex-wrap gap-2">
          {["iPhone 14 écran cassé", "Samsung S22 batterie", "iPad ne s'allume pas"].map(
            (sugg, idx) => (
              <button
                key={idx}
                onClick={() => {
                ...
                className="text-xs bg-white border rounded-full px-3 py-1 hover:border-orange-300"
              >
                {sugg}
              </button>
            )
      ...
      <div className="border-t p-3 bg-white">
        <div className="flex gap-2">
          <input
            ...
            placeholder="🔧 Décrivez l'appareil et la panne..."
            className="flex-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500"
      ...
      {detectedMultiRepairs.length > 0 && (
        <div className="border-t p-3 bg-orange-50">
          <button
            ...
            className="w-full bg-orange-500 text-white py-2 rounded-lg text-sm font-semibold"
          >
            📝 Remplir {detectedMultiRepairs.length} appareils
          </button>
        </div>
      )}
    </div>
